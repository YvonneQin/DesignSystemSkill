// Add left + right icons to *Tag* / Checkable — same logic as Colorful/Button.
...
const set = figma.getNodeById('524:20');
if (!set) throw new Error('Checkable tag set missing');
...
for (let oi = 0; oi < order.length; oi++) {
  ...
  for (let i = 0; i < set.children.length; i++) {
    ...
    if (c.name.indexOf('Checked=' + val) != -1) {
      c.x = PAD;
      c.y = rowY;
    }
  }
